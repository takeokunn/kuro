...
;; Wrappers around the Rust FFI for querying DEC private mode state,
;; mouse tracking mode, and Kitty keyboard protocol flags.
;;
;; All getters are generated by `kuro--define-ffi-getters' and follow
;; the guard/catch/fallback pattern from `kuro-ffi'.  Two consolidated
;; endpoints (`kuro--get-cursor-state', `kuro--get-terminal-modes')
;; batch multiple queries into a single FFI call for lower overhead.
