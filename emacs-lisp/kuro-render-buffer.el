...
(declare-function kuro--unpack-ffi-face-range "kuro-overlays" (range))
    ...
    (save-excursion
      (let ((inhibit-read-only t)
        ...
        (let* ((line-start (point))
                 ;; Capture line-end BEFORE delete+insert for blink overlay bounds.
                 ;; +1 to include the trailing newline in the overlay scan range.
                 (line-end-before (1+ (line-end-position)))
                 (remaining nil))
            ;; Inline blink-overlay clear: avoids a separate navigation call.
      ...
      (when cursor-pos
        ...
        (if (kuro--get-cursor-visible)
            ...
            (setq-local cursor-type
                        (kuro--decscusr-to-cursor-type
                         (or (kuro--get-cursor-shape) 0)))
