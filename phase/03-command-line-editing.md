...
- [x] **CSIカーソル移動シーケンスの実装**
  ...
  - [x] VPA (d) - 垂直位置絶対
  - [x] CHA (G) - カーソル水平絶対
  - [x] 境界クランプ（カーソルは有効範囲内に留まる）

- [x] **ED（消去表示）シーケンスの実装**
  - [x] パラメータ0: カーソルから画面末尾まで消去
  - [x] パラメータ1: 画面先頭からカーソルまで消去
  - [x] パラメータ2: 画面全体を消去
  - [x] 影響を受ける行をダーティとしてマーク

- [x] **EL（消去行）シーケンスの実装**
  - [x] パラメータ0: カーソルから行末まで消去
  - [x] パラメータ1: 行先頭からカーソルまで消去
  - [x] パラメータ2: 行全体を消去
  - [x] 行をダーティとしてマーク

- [x] **キーボードからPTYバイトへのマッピング実装**
  - [x] 矢印キー（上/下/左/右）
  - [x] 特殊キー（RET, DEL, TAB）
  - [x] Controlキーの組み合わせ（C-c, C-d, C-z）
  - [x] Alt/Meta修飾子の処理
