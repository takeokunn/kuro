...
#[defun]
fn kuro_core_get_mouse_mode<'e>(env: &'e Env, session_id: u64) -> EmacsResult<Value<'e>> {
    query_session(env, session_id, 0i64, |s| Ok(s.get_mouse_mode() as i64))
}

/// Get mouse SGR extended coordinates modifier state (t if active, nil if not)
#[defun]
fn kuro_core_get_mouse_sgr<'e>(env: &'e Env, session_id: u64) -> EmacsResult<Value<'e>> {
...
#[defun]
fn kuro_core_get_mouse_pixel<'e>(env: &'e Env, session_id: u64) -> EmacsResult<Value<'e>> {
