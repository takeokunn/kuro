...
#[defun]
fn kuro_core_test_feed(env: &Env, data: String) -> EmacsResult<Value<'_>> {
    catch_panic_test(env, || {
        let bytes = data.into_bytes();
        let mut global = lock_test!(mut global);
        global.as_mut().map_or(Ok(false), |terminal| {
...
#[defun]
fn kuro_core_test_resize(env: &Env, rows: u16, cols: u16) -> EmacsResult<Value<'_>> {
    catch_panic_test(env, || {
        let mut global = lock_test!(mut global);
