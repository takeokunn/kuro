...
fn test_encode_line_single_cell() {
    ...
    assert_eq!(encoded.face_ranges.len(), 1);
    assert_face_range!(encoded.face_ranges, 0: buf 0, 1, fg 0xFF00_0000u32, bg 0xFF00_0000u32, flags 0u64);
...
fn test_encode_line_combining_char_at_last_position_face_range_end() {
    ...
    assert_eq!(
        encoded.face_ranges.len(),
        1,
        "identical attrs must merge into one range"
    );
    let range = &encoded.face_ranges[0];
...
fn test_encode_line_with_pool_single_cell_matches_encode_line() {
    ...
    let encoded_pool = encode_line_with_pool(cells, false, &mut pool);
    let encoded_line = encode_line(cells);
    assert_eq!(
