...
//!   `NOTIFICATION_MAX_BYTES` = 4096 (desktop notification body/title cap)
...
#[test]
// VALUE: NOTIFICATION_MAX_BYTES must be exactly 4096 (4 KiB).
fn test_notification_max_bytes_is_4096() {
    assert_eq!(
        NOTIFICATION_MAX_BYTES, 4096,
        "NOTIFICATION_MAX_BYTES must equal 4096 bytes (4 KiB)"
    );
}
