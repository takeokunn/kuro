...
fn test_el_mode1_at_last_column_clears_entire_line() {
    let mut term = crate::TerminalCore::new(3, 10);
    let row = 0;
    fill_cells!(term, row row, cols 0..10, 'U');
...
fn test_el_mode0_at_column_zero_clears_entire_line() {
    let mut term = crate::TerminalCore::new(3, 10);
    let row = 1;
    fill_cells!(term, row row, cols 0..10, 'G');
...
fn test_el_modes_round_trip_on_same_row() {
    ...
    let mut term = crate::TerminalCore::new(3, 10);
    fill_cells!(term, row row, cols 0..10, 'A');
    term.screen.move_cursor(row, 4);
    ...
    fill_cells!(term2, row row, cols 0..10, 'B');
    ...
    fill_cells!(term3, row row, cols 0..10, 'C');
...
fn test_ech_clears_cells_from_cursor() {
    let mut term = crate::TerminalCore::new(3, 10);
    let row = 0;
    fill_cells!(term, row row, cols 0..10, 'H');
...
fn test_ech_bce_applies_background_color() {
    ...
    let row = 1;
    fill_cells!(term, row row, cols 0..10, 'I');
...
fn test_el_unknown_mode_is_noop() {
    let mut term = crate::TerminalCore::new(3, 10);
    let row = 0;
    fill_cells!(term, row row, cols 0..10, 'K');
