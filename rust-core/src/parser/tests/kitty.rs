...
fn test_transmit_rgb_2x2_image() {
    ...
    match result.unwrap() {
        KittyCommand::Transmit { format, pixel_width, pixel_height, pixels, .. } => {
            ...
            assert_eq!(pixels.len(), 12, "2×2 RGB = 12 bytes");
        }
        other => panic!("expected Transmit, got {other:?}"),
    }
}
...
fn test_transmit_rgba_2x2_image() {
    ...
    match result.unwrap() {
        KittyCommand::Transmit { format, pixel_width, pixel_height, pixels, .. } => {
            ...
            assert_eq!(pixel_width, 2);
            assert_eq!(pixel_height, 2);
            assert_eq!(pixels.len(), 16, "2×2 RGBA = 16 bytes");
        }
        other => panic!("expected Transmit, got {other:?}"),
    }
}
...
#[test]
fn test_kitty_png_rgb_black_pixel_round_trips() {
    let b64 = encode_1x1_png_b64(png::ColorType::Rgb, &[0x00, 0x00, 0x00]);
    let mut chunk_state = None;
    let payload = format!("a=t,f=100,i=70,s=1,v=1;{b64}");
    let result = process_apc_payload(payload.as_bytes(), &mut chunk_state);
    match result.unwrap() {
...
#[test]
fn test_kitty_png_rgba_white_pixel_round_trips() {
    let b64 = encode_1x1_png_b64(png::ColorType::Rgba, &[0xFF, 0xFF, 0xFF, 0xFF]);
    let mut chunk_state = None;
    let payload = format!("a=t,f=100,i=71,s=1,v=1;{b64}");
    let result = process_apc_payload(payload.as_bytes(), &mut chunk_state);
    match result.unwrap() {
        KittyCommand::Transmit { pixels, format, .. } => {
            assert_eq!(format, ImageFormat::Rgba);
