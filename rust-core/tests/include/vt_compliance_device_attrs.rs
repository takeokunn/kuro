...
// === XTWINOPS (CSI Ps t) — window size reports ===

/// CSI 18 t reports text-area size in characters: CSI 8 ; rows ; cols t
...
/// geometry, so it answers CSI 4 ; 0 ; 0 t (0 = unknown, per the same rule the
/// in-band resize report uses).
#[test]
...
#[test]
fn vt_xtwinops_18_reflects_resize() {
    let mut t = TerminalCore::new(24, 80);
    t.resize(30, 100);
    t.advance(b"\x1b[18t");
    assert_eq!(
        t.pending_responses().to_vec(),
        vec![b"\x1b[8;30;100t".to_vec()]
    );
}

/// Window-manipulation and host-revealing XTWINOPS ops are ignored (no
/// response). This prevents applications from moving, resizing, iconifying, or
/// interrogating the host Emacs window via terminal output (cf. xterm
/// `allowWindowOps`). Covers: de-iconify(1), iconify(2), move(3),
/// resize-pixels(4), resize-chars(8), maximize(9), report-position(13),
/// report-title(21), and the empty CSI t.
#[test]
fn vt_xtwinops_manipulation_and_host_ops_ignored() {
    let manipulation_ops: [&[u8]; 9] = [
        b"\x1b[1t",         // de-iconify
        b"\x1b[2t",         // iconify
        b"\x1b[3;0;0t",     // move window
        b"\x1b[4;500;500t", // resize in pixels
        b"\x1b[8;50;50t",   // resize in characters
        b"\x1b[9;1t",       // maximize
        b"\x1b[13t",        // report window position (privacy)
        b"\x1b[21t",        // report window title (exfiltration)
        b"\x1b[t",          // no params
    ];
    for seq in manipulation_ops {
        let mut t = TerminalCore::new(24, 80);
        t.advance(seq);
        assert!(
            t.pending_responses().is_empty(),
            "XTWINOPS {seq:?} must not produce a response (manipulation/host op)"
        );
    }
}
