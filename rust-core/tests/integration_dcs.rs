...
fn xtgettcap_colors_responds_with_256() {
    ...
    let responses = common::read_responses(&t);
    assert!(!responses.is_empty(), "XTGETTCAP for 'colors' must produce a response");
    let resp = &responses[0];
    assert!(
        resp.contains("1+r"),
...
fn xtgettcap_co_alias_responds() {
    ...
    let responses = common::read_responses(&t);
    assert!(!responses.is_empty(), "XTGETTCAP for 'Co' must produce a response");
    let resp = &responses[0];
    assert!(
        resp.contains("1+r"),
...
fn dcs_unknown_passthrough_does_not_panic() {
    ...
    assert_eq!(t.cursor_row(), row_before, "unknown DCS must not move cursor row");
    assert_eq!(t.cursor_col(), col_before, "unknown DCS must not move cursor col");
}
...
fn kitty_png_format_transmit_and_display_queues_notification() {
    ...
    let mut png_buf: Vec<u8> = Vec::new();
    {
        ...
        writer.write_image_data(&[0x80, 0x40, 0x20]).expect("PNG pixels");
    }
    ...
    assert!(!png_b64.is_empty(), "kitty PNG image must be stored after a=T");
}
...
fn kitty_transmit_and_display_q2_queues_notification_suppresses_response() {
    ...
    assert_eq!(notifs[0].image_id, 50, "notification must reference image 50");
}
...
fn kitty_three_sequential_images_have_unique_ids() {
    ...
    let notifs = t.pending_image_notifications();
    assert_eq!(notifs.len(), 3, "three a=T sequences must queue three notifications");
    ...
    assert!(!t.get_image_png_base64(61).is_empty(), "image 61 must be stored");
    assert!(!t.get_image_png_base64(62).is_empty(), "image 62 must be stored");
    assert!(!t.get_image_png_base64(63).is_empty(), "image 63 must be stored");
}
...
fn kitty_place_with_xy_pixel_offsets_queues_notification() {
    ...
    let notifs = t.pending_image_notifications();
    assert_eq!(notifs.len(), 1, "a=p with X/Y offsets must queue exactly one notification");
    assert_eq!(notifs[0].image_id, 65, "notification must reference image 65");
...
fn sixel_notification_has_nonzero_dimensions() {
    ...
    let notifs = t.pending_image_notifications();
    assert_eq!(notifs.len(), 1, "sixel must produce exactly one notification");
    assert!(
...
fn sixel_multi_color_palette_stores_image() {
    ...
    let notifs = t.pending_image_notifications();
    assert_eq!(notifs.len(), 1, "multi-color sixel must produce exactly one notification");

    let png_b64 = t.get_image_png_base64(notifs[0].image_id);
    assert!(!png_b64.is_empty(), "multi-color sixel image must be stored");
}
