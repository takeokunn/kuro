...
(ert-deftest kuro-module-lib-name-matches-tier-functions ()
  ...
  (kuro-module-test--with-dev-stubs
    (let ((lib-name (kuro-module--lib-name))
          (dev-path (kuro-module-test--candidate-path
...
(ert-deftest kuro-module-test--module-load-calls-module-load-when-file-found ()
  ...
  (kuro-module-test--with-temp-dir (tmpdir "kuro-load-")
    (let* ((module-path (expand-file-name (kuro-module--lib-name) tmpdir))
           (_ (kuro-module-test--write-valid-library-file module-path))
           (candidate (kuro-module-test--candidate-for-path module-path))
           (loaded-path nil)
           (was-bound (fboundp 'kuro-core-init))
           (original-def (and was-bound (symbol-function 'kuro-core-init))))
      ...
      (unwind-protect
          (cl-letf (((symbol-function 'kuro-module--find-library)
            ...
            (kuro-module-load)
            (should (equal loaded-path module-path)))
        (when was-bound
...
(ert-deftest kuro-module-test--module-load-message-fmt-contains-path ()
  ...
  (kuro-module-test--with-temp-dir (tmpdir "kuro-load-message-")
    (let* ((module-path (expand-file-name (kuro-module--lib-name) tmpdir))
           (_ (kuro-module-test--write-valid-library-file module-path))
           (candidate (kuro-module-test--candidate-for-path module-path))
           (captured-fmt nil)
      ...
      (unwind-protect
          (cl-letf (((symbol-function 'kuro-module--find-library)
            ...
            (should (equal captured-arg module-path)))
        (when was-bound
          (fset 'kuro-core-init original-def))))))

(ert-deftest kuro-module-test--module-load-rejects-raw-string-candidate ()
  "`kuro-module-load' rejects stale callers that return an untyped raw path."
  (let* ((module-load-called nil)
    ...
    (unwind-protect
        (cl-letf (((symbol-function 'kuro-module--find-library)
                  ...
                  ((symbol-function 'module-load)
                   (lambda (_) (setq module-load-called t))))
          (should-error (kuro-module-load) :type 'error)
          (should-not module-load-called))
      (when was-bound
        (fset 'kuro-core-init original-def)))))

(ert-deftest kuro-module-test--module-load-revalidates-candidate-mode ()
  "`kuro-module-load' revalidates the candidate immediately before module-load."
  (kuro-module-test--with-temp-dir (tmpdir "kuro-load-drift-")
    (let* ((module-path (expand-file-name (kuro-module--lib-name) tmpdir))
           (_ (kuro-module-test--write-valid-library-file module-path))
           (candidate (kuro-module-test--candidate-for-path module-path))
           (module-load-called nil)
           (was-bound (fboundp 'kuro-core-init))
           (original-def (and was-bound (symbol-function 'kuro-core-init))))
      (set-file-modes module-path #o644)
      (when was-bound (fmakunbound 'kuro-core-init))
      (unwind-protect
          (cl-letf (((symbol-function 'kuro-module--find-library)
                     (lambda () candidate))
...
(ert-deftest kuro-module-test--module-try-empty-string-file-exists-stubbed-nil ()
  "`kuro--module-try' returns nil for an empty string."
...
(ert-deftest kuro-module--verify-sha256-nil-hash-errors ()
  "`kuro-module--verify-sha256' rejects a missing expected hash."
  (should-error (kuro-module--verify-sha256 "/any/path" nil) :type 'error))

(ert-deftest kuro-module--verify-sha256-raw-string-hash-errors ()
  "`kuro-module--verify-sha256' rejects raw string hashes at the boundary."
  (should-error
   (kuro-module--verify-sha256 "/any/path" (make-string 64 ?0))
   :type 'error))
...
(ert-deftest kuro-module--verify-sha256-matching-hash-returns-t ()
  ...
  (kuro-module-test--with-temp-file (tmpfile "kuro-test-sha256-")
    (let* ((content "hello kuro")
           ...
           (expected-hash (kuro-module--parse-sha256 expected "test")))
      (with-temp-file tmpfile (insert content))
      (should (kuro-module--verify-sha256 tmpfile expected-hash)))))
  ...
  (kuro-module-test--with-temp-file (tmpfile "kuro-test-sha256-")
    ...
    (should-not
     (kuro-module--verify-sha256
      tmpfile
      (kuro-module--parse-sha256 (make-string 64 ?0) "test")))))
...
(ert-deftest kuro-module-try-expansion-guards-with-candidate-validator ()
  "`kuro--module-try' body routes paths through the candidate validator."
  (let* ((exp (macroexpand-1
               '(kuro--module-try "/some/path")))
         (body (caddr exp)))
    (should (eq (car body) 'and))
