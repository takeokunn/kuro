...
(ert-deftest kuro-poll-modes-clipboard-write-then-query-both-processed ()
  ...
  (kuro-poll-test--with-buffer
    (let ((kill-ring-text nil)
          ...
          (kuro-clipboard-policy 'allow))
      (cl-letf (((symbol-function 'kuro--poll-clipboard-actions)
                 (lambda () '((write "written" "clipboard")
                              (query nil "clipboard"))))
                ((symbol-function 'kill-new)
...
(ert-deftest kuro-poll-modes-dispatch-clipboard-action-rejects-legacy-cons ()
  "`kuro--dispatch-clipboard-action' rejects legacy cons actions."
  (let ((called nil))
    (cl-letf (((symbol-function 'kuro--clipboard-write)
...
(ert-deftest kuro-poll-modes-dispatch-clipboard-action-rejects-malformed-actions ()
  "`kuro--dispatch-clipboard-action' rejects unsupported OSC 52 action shapes."
  (let ((called nil)
        (actions '(nil
                   atom
                   (paste "x" "clipboard")
                   ...
                   (write nil "clipboard")
                   (write "x")
                   (write "x" "clipboard" extra)
                   (query)
                   (query "payload" "clipboard")
                   (query nil)
                   (query nil "clipboard" extra)
                   (write "x" "bogus")
                   (write "x" :clipboard))))
    (cl-letf (((symbol-function 'kuro--clipboard-write)
               (lambda (&rest _) (setq called t)))
