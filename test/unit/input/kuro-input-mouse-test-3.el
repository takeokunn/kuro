;;; kuro-input-mouse-test-3.el --- ERT tests for kuro-input-mouse.el — Groups 23+  -*-  lexical-binding: t; -*-
...
The macro expansion produces a defun, so the symbol is fboundp afterward."
  ...
  (kuro--def-mouse-cmd kuro--test-mouse-cmd-probe
    42
    t
...
dispatch macro resolves to a single kuro--send-key call with the right bytes."
  ...
  (kuro--def-mouse-cmd kuro--test-mouse-press-btn1
    1
    t
...
The release path must produce a lowercase 'm' terminator in SGR mode."
  (kuro--def-mouse-cmd kuro--test-mouse-release-btn2
    2
    nil
...
kuro--dispatch-mouse-event guards on (> kuro--mouse-mode 0)."
  (kuro--def-mouse-cmd kuro--test-mouse-press-noop
    0
    t
...
X10 release always substitutes 3 for the button number (btn-byte = 3+32 = 35)."
  (kuro--def-mouse-cmd kuro--test-mouse-release-x10
    0
    nil
...
;;; Group 25: kuro--def-scroll-command macro — structural coverage

(ert-deftest kuro-input-mouse-def-scroll-command-expands-to-defun ()
  "`kuro--def-scroll-command' single-step expands to a `defun' form."
  (let ((exp (macroexpand-1
              '(kuro--def-scroll-command kuro-test--fake-scroll
                 "Test scroll." (ignore) 0))))
    (should (eq (car exp) 'defun))
    (should (eq (cadr exp) 'kuro-test--fake-scroll))))

(ert-deftest kuro-input-mouse-def-scroll-command-expansion-has-interactive ()
  "`kuro--def-scroll-command' expansion contains `(interactive)'."
  (let ((exp (macroexpand-1
              '(kuro--def-scroll-command kuro-test--fake-scroll2
                 "Test." (ignore) 0))))
    (should (member '(interactive) (cddr exp)))))

(ert-deftest kuro-input-mouse-def-scroll-command-generated-cmd-is-fboundp ()
  "`kuro--mouse-scroll-up--scrollback' (generated by macro) is defined."
  (should (fboundp #'kuro--mouse-scroll-up--scrollback)))

(ert-deftest kuro-input-mouse-def-scroll-command-generated-cmd-is-interactive ()
  "`kuro--mouse-scroll-up--scrollback' is an interactive command."
  (should (commandp #'kuro--mouse-scroll-up--scrollback)))

(provide 'kuro-input-mouse-test)

(provide 'kuro-input-mouse-test-3)
;;; kuro-input-mouse-test-3.el ends here
