...
(ert-deftest kuro-faces-test--char-width-overrides-match-probe-chars ()
  ...
  (let ((cwt-ranges (mapcar (lambda (r) (cons (car r) (cdr r)))
    ...
    (should (member '(#x2500 . #x257F) cwt-ranges))
    (should (member '(#x2580 . #x259F) cwt-ranges))
    (should (member '(#x25A0 . #x25FF) cwt-ranges))))
...
(ert-deftest test-kuro-refine-glyph-widths-iterates-both-passes ()
  ...
  (let (called-chars)
    (cl-letf (((symbol-function 'display-graphic-p) (lambda () t))
      ...
      (kuro--refine-glyph-widths)
      ;; Pass 1: one probe char per kuro--char-width-overrides entry
      (let ((pass1-chars (mapcar #'cdr kuro--glyph-probe-chars)))
        (dolist (c pass1-chars)
