...
(ert-deftest kuro-render-buffer-update-row-position-cache-after-line-change-nil-positions ()
  "Does not error when `kuro--row-positions' is nil."
  (kuro-render-buffer-test--with-buffer
    (setq kuro--row-positions nil)
    (kuro--update-row-position-cache-after-line-change 0 1 3 99)
    (should (null kuro--row-positions))))

(ert-deftest kuro-render-buffer-update-row-position-cache-after-line-change-last-row ()
  "Changing the last row does not crash even when row+1 >= vector length."
  (kuro-render-buffer-test--with-buffer
    (setq kuro--row-positions [10 20])
    ;; row=1 is the last entry; row+1=2 >= length=2, so body is skipped.
    (kuro--update-row-position-cache-after-line-change 1 1 5 99)
    (should (equal kuro--row-positions [10 20]))))

(ert-deftest kuro-render-buffer-update-row-position-cache-after-line-change-skips-nil-entries ()
  "Nil entries in the loop range are left as nil rather than adjusted."
  (kuro-render-buffer-test--with-buffer
    (setq kuro--row-positions [10 20 nil 40])
    ;; row=0, old-len=1, new-len=3 → delta=+2, new-line-end=99
    ;; rp[1] = (1+ 99) = 100; rp[2] = nil (skipped); rp[3] = 40+2 = 42.
    (kuro--update-row-position-cache-after-line-change 0 1 3 99)
    (should (equal kuro--row-positions [10 100 nil 42]))))
